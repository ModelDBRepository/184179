'''
Current clamp demo using Allen Brain's model 472306544.
...
from neuron472306544 import Neuron472306544
...
def demo(iapp):
    ...
    h.celsius = 34.0
    cell = Neuron472306544(name='neuron')
    ...
    vbox.intercept(0)
    vbox.map('Allen Institute model 472306544; v includes junction potential', 10, 10, 600, 400)

    # procedure for doing and plotting each simulation
    def do_current_clamp_experiment(amp, color):   
        ic.amp = amp / 1000.
        
        h.finitialize(-92.3040847778)
        h.fcurrent()
